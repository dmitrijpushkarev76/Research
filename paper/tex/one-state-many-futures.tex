...
Assume \textup{(H1)--(H3)}; in particular $f^{-1}(0)=\{0\}$, which is part of
\textup{(H1)}. Then, with $\Hmax(0)$ the forward-maximal solutions of
Definition~\ref{def:classes} and not the two-sided maximal solutions of
Section~\ref{sec:senses},
\[
  \Hglob(0)=\{x_{\infty}\},
  \qquad
...
on $I$. Then $x$ has the finite right endpoint $T\notin I$ at which it remains
finite --- indeed $x(t)\to0$ as $t\uparrow T$ --- so setting $x(T)=0$ extends it
continuously, and the extension solves \eqref{eq:ivp} at $T$ because $f(0)=0$.
(In general this is the continuation principle used in Step 4: at a finite right
endpoint where the solution stays finite, Peano's theorem supplies a local
solution from the limiting value, so the solution extends. Here the limiting
value is the equilibrium and the extension is explicit.) Either way maximality
is contradicted.
