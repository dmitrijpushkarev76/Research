...
$x\equiv 0$. It also has continuum-many solutions that admit no forward
extension: for each waiting time $T\ge 0$ there is a solution that rests at the
origin until $T$, departs, and reaches $+\infty$ at $T+3\sqrt{2}\pi/4$. Every
non-trivial branch terminates in finite time, which is exactly why the first
count is one; the second count is $\cont$ for an independent reason, that the
equilibrium can be left at any moment. Each hypothesis moves one count, and only
their conjunction separates the two.
...
That closed form is a convenience, and not a reason to prefer this $f$ to other
members of the class. Some of them do better. For $f(x)=\sqrt{|x|}\,(1+|x|)$ one
has $F(x)=2\arctan\sqrt{x}$, so $T_{*}=\pi$ exactly, and every branch is
elementary: $x_{T}(t)=\tan^{2}\!\big((t-T)/2\big)$ on $(T,T+\pi)$. The $f$ used
here admits no such closed form for $\varphi$ itself, which is why
Figure~\ref{fig:branches} is drawn by inverting $F$ numerically. Nothing in what
follows depends on the choice; any member of the class would serve.
...
Assume \textup{(H1)--(H3)} and let $f^{-1}(0)=\{0\}$. Then
\[
  \Hglob(0)=\{x_{\infty}\},
  \qquad
...
on $I$, which extends by $0$ --- contradicting maximality.
...
\smallskip
\emph{Step 7 (the two counts).} Steps 3, 5 and 6 together give the stated
description of $\Hmax(0)$. The map $T\mapsto x_{T}$ is injective: if
$T_{1}<T_{2}$ then $x_{T_{1}}>0=x_{T_{2}}$ on $(T_{1},T_{2}]$, an interval of
positive length. Hence $|\Hmax(0)|\ge|[0,\infty)|=\cont$. Conversely a solution
is a continuous real function on a subinterval of $[0,\infty)$; there are
$\cont$ such subintervals, and a continuous function on an interval is
determined by its values on a countable dense set, so each interval carries at
most $\cont^{\aleph_{0}}=\cont$ of them. Thus
$|\Hmax(0)|\le\cont\cdot\cont=\cont$, and equality holds.

Finally, each $x_{T}$ with $T<\infty$ has bounded domain and so is not globally
defined, while $x_{\infty}\in\Hglob(0)$. Since
$\Hglob(0)\subseteq\Hmax(0)$, this leaves $\Hglob(0)=\{x_{\infty}\}$.
\end{proof}
...
uniqueness criterion, in the Osgood \cite{Osgood} and Kamke line; see Hartman
\cite[ch.~III \S6]{Hartman} and, for a modern restatement citing Binding,
Fjordholm \cite{Fjordholm}. Case (6.5) is exactly hypothesis (H3): when
...
is a duality --- $\hat f(\alpha)=\alpha^{2}f(1/\alpha)$ turns escape to infinity
into non-uniqueness at the origin --- so the observation that (H2) and (H3) are
the two ends of one integral is his; he credits awareness of it to Wintner and
...
exact dichotomy. The general fact that non-uniqueness forces continuum-many
solutions --- our Step 7, in far greater generality --- is Kneser's theorem,
\cite[ch.~II \S4 Thm.~4.1]{Hartman}.

\textbf{Drivas and Mailybaev \cite{DrivasMailybaev}, 2018.} State the
continuum-many waiting-time solutions explicitly for $\dot x=x^{1/3}$: ``a
continuum infinity of solutions, which stay at the origin until an arbitrary
time $t_{1}$ and peal [sic] off.'' There the branches are global, so no gap
between the two counts arises.
