...
% EJPS/Springer Basic house style: author-date, "(Author Year)" with no comma.
...
classical: it was characterised by Wallach in 1948 and treated in general form
by Binding in 1979. This note takes one concrete
...
(2007), Smeenk and W\"uthrich (2021) and Doboszewski (2020), for Newtonian
blow-up by Wilson (2009), and within a broader formulation of indeterminism by
Azhar and Namjoo (2021).
...
through an equilibrium was characterised by \citet{Wallach}, and
...
--- is given by \citet{Binding}. The conceptual point is also familiar.
...
settled one \citep{Earman,SmeenkWuthrich}. In Newtonian mechanics \citet{Wilson} argues that failures of global existence are descriptive gaps
rather than failures of determinism, in explicit opposition to Earman and
Norton. \citet{AzharNamjoo} argue that a uniqueness-based
...
\node[anchor=east, font=\footnotesize] at (axis cs:3.085,4.2) {$T=0$};
\node[anchor=east, font=\footnotesize] at (axis cs:4.685,4.2) {$T=1.6$};
\node[anchor=east, font=\footnotesize] at (axis cs:6.285,4.2) {$T=3.2$};
\node[anchor=east, font=\footnotesize] at (axis cs:7.885,4.2) {$T=4.8$};
\end{axis}
\end{tikzpicture}
...
Definition~\ref{def:solution} asks for a differentiable solution; \citet{Binding} works instead with absolutely continuous $x$ satisfying
...
\begin{itemize}
  ...
  \item \textbf{Maximal (extremal):} the \emph{largest} solution pointwise on
        ...
        $x_{\infty}$. \citet[ch.~III \S2]{Hartman} uses the word in this
        sense.
\end{itemize}
...
\textbf{\citet{Wallach}.} Characterised \emph{all} solutions of
$\dot x=f\circ x$, $x(0)=0$ for continuous $f$. \citet[p.~184]{Binding} reports this --- ``Wallach characterized all solutions of (1.1)'' ---
...
\textbf{\citet[\S3, pp.~186--188]{Binding}.} The machinery of
Theorem~\ref{thm:counts}. Lemma 3.2: every solution is monotonic, which Binding
credits to \citet{Wallach} for continuous $f$ --- our Step 1.
...
\textbf{\citet[\S\S5--6, pp.~190--193]{Binding}.} Theorem 5.3 is the
uniqueness criterion, in the \citet{Osgood} and Kamke line --- Binding
names both on the same page --- and see \citet[ch.~III \S6]{Hartman},
where Theorem 6.1 is Kamke's and Corollary 6.2 is Osgood's. \citet{Fjordholm}, whose own subject is Filippov uniqueness for discontinuous
...
\textbf{\citet[Lemma 7.5, p.~194]{Binding}.} The structure theorem: every
...
\textbf{\citet[ch.~III \S5 Thm.~5.1, p.~30]{Hartman}.} A theorem of
...
$\dot x=\sqrt{|x|}$, at \citep[][ch.~I \S1, eq.~(1.5), p.~2]{Hartman}, whose
...
\citep[][ch.~II \S4 Thm.~4.1]{Hartman}.

\textbf{\citet{DrivasMailybaev}.} Cited here for continuation past a non-Lipschitz singularity and the
...
\textbf{\citet{Berndl}.} Use ``maximal (non-extendible)
...
surveyed in \citet{AgarwalLakshmikantham}.
...
disagree about. \citet[p.~2]{Doboszewski} adopts the criterion of
\citet{Butterfield} --- his DM2 --- under which a theory is deterministic for a
...
\citet{MullerPlacek} separate three families of definition ---
...
Norton's dome \citep{Norton}, where the wait-then-depart branches are global and
...
close to the position \citet[p.~182]{Wilson} defends for Newtonian
...
Earman's phrase \citep[][\S6.4]{Earman}, often ``not maximal simpliciter'', and the
larger extensions are not unique. \citet{SmeenkWuthrich}
...
\citep[][p.~12]{Doboszewski} of spacetimes that simply stop --- Minkowski space
...
trick'' \citep[][p.~2]{Doboszewski}. We claim no priority over that discussion and
...
in the first place. \citet{AzharNamjoo} propose that
...
What the example is for, then, is not adjudication. It is that the two readings
return different verdicts here at all. The standard illustrations do not
separate them. On Norton's dome \citep{Norton} the wait-then-depart branches are
defined at every later time, so $\Hglob$ and $\Hmax$ coincide there and
Readings~1 and~2 agree that the state has many futures; the disagreement that
example generates is about whether those branches are legitimate, not about
which class to count. In the non-collision singularities of \citet{Xia} there is
finite-time escape but the field is locally Lipschitz away from collisions, so
nothing branches and the question does not arise. Here $|\Hglob(0)|=1$ while
$|\Hmax(0)|=\cont$, so a criterion quantifying over the one class and a criterion
quantifying over the other disagree about one state of one system, with every
quantity exact. That is what the example supplies to a debate it does not
settle: a place where the choice of class is doing all of the work, and can be
seen doing it.
...
belongs to the philosophical discussion rather than to the mathematics: it is a
state at which criteria quantifying over the two classes return different
verdicts, which the standard illustrations do not supply.
Section~\ref{sec:determinism} sets that out. The second is Proposition~\ref{prop:locality}, which says where
...
Norton's construction \citep{Norton} places a particle on a frictionless surface
...
discussed at length by \citet{Malament} and \citet{Fletcher}, and more
recently by \citet{Berry}, who sets it beside an exactly solvable quantum
counterpart showing no trace of the phenomenon; see also
\citet{BhatBernstein}.
...
\citep{Norton} makes $|h'|\le1$ mandatory, since a sine is at most $1$; the same
...
systems, and the responses available to it, are discussed by \citet{Earman}.
...
question the dome literature asks about $\sqrt r$ \citep{Fletcher,Norton}; we
take no position.

%------------------------------------------------------------------
\begin{thebibliography}{}

\bibitem[Agarwal and Lakshmikantham(1993)]{AgarwalLakshmikantham}
Agarwal, R.P., \& Lakshmikantham, V. (1993). \emph{Uniqueness and nonuniqueness
criteria for ordinary differential equations}. Series in Real Analysis 6.
Singapore: World Scientific.

\bibitem[Azhar and Namjoo(2021)]{AzharNamjoo}
Azhar, F., \& Namjoo, M.H. (2021). Spacetime singularities and a novel
formulation of indeterminism. arXiv:2101.10887.

\bibitem[Berndl et al.(1995)]{Berndl}
Berndl, K., D\"urr, D., Goldstein, S., Peruzzi, G., Zangh\`i, N. (1995). On the
global existence of Bohmian mechanics. \emph{Communications in Mathematical
Physics}, 173, 647--673.

\bibitem[Berry(2025)]{Berry}
Berry, M.V. (2025). Newtonian and quantum (in)determinism: Norton's dome and
cosmic chaos. \emph{European Journal of Physics}, 46(4), 045002.

\bibitem[Bhat and Bernstein(1997)]{BhatBernstein}
Bhat, S.P., \& Bernstein, D.S. (1997). Example of indeterminacy in classical
dynamics. \emph{International Journal of Theoretical Physics}, 36, 545--550.
...
\bibitem[Binding(1979)]{Binding}
Binding, P. (1979). The differential equation $\dot x=f\circ x$. \emph{Journal
of Differential Equations}, 31, 183--199.

\bibitem[Butterfield(1989)]{Butterfield}
Butterfield, J. (1989). The hole truth. \emph{British Journal for the Philosophy
of Science}, 40(1), 1--28.

\bibitem[Doboszewski(2020)]{Doboszewski}
Doboszewski, J. (2020). Epistemic holes and determinism in classical general
relativity. \emph{British Journal for the Philosophy of Science}, 71,
1093--1111. Page references in the text are to the preprint, whose pagination
differs from the journal's.

\bibitem[Drivas and Mailybaev(2021)]{DrivasMailybaev}
Drivas, T.D., \& Mailybaev, A.A. (2021). ``Life after death'' in ordinary
differential equations with a non-Lipschitz singularity. \emph{Nonlinearity},
34, 2296--2326. Preprint arXiv:1806.09001 (2018).

\bibitem[Earman(2007)]{Earman}
Earman, J. (2007). Aspects of determinism in modern physics. In Butterfield, J.,
\& Earman, J. (Eds.) \emph{Handbook of the philosophy of science: Philosophy of
physics} (pp. 1369--1434). Amsterdam: North-Holland.
% TODO: add a page or section reference for the unbounded-below potential
% discussion. The earlier wording attributed to that chapter a specific
% objection; that attribution was not located in the chapter and has been
% weakened.

\bibitem[Fjordholm(2018)]{Fjordholm}
Fjordholm, U.S. (2018). Sharp uniqueness conditions for one-dimensional,
autonomous ordinary differential equations. arXiv:1804.08377.

\bibitem[Fletcher(2012)]{Fletcher}
Fletcher, S.C. (2012). What counts as a Newtonian system? The view from Norton's
dome. \emph{European Journal for Philosophy of Science}, 2, 275--297.

\bibitem[Hartman(1964)]{Hartman}
Hartman, P. (1964). \emph{Ordinary differential equations}. New York: Wiley.

\bibitem[Malament(2008)]{Malament}
Malament, D.B. (2008). Norton's slippery slope. \emph{Philosophy of Science},
75, 799--816.
% TODO: author must confirm the bibliographic details against the printed
% article before submission. Cited here as a discussion of the dome only; the
% article has not been read.

\bibitem[M\"uller and Placek(2018)]{MullerPlacek}
M\"uller, T., \& Placek, T. (2018). Defining determinism. \emph{British Journal
for the Philosophy of Science}, 69, 215--252. Section and footnote references in
the text are to the preprint (PhilSci-Archive 11437, 27 April 2015), whose
numbering may differ from the journal's.

\bibitem[Norton(2008)]{Norton}
Norton, J.D. (2008). The dome: An unexpectedly simple failure of determinism.
\emph{Philosophy of Science}, 75, 786--798.

\bibitem[Osgood(1898)]{Osgood}
Osgood, W.F. (1898). Beweis der Existenz einer L\"osung der
Differentialgleichung $dy/dx=f(x,y)$ ohne Hinzunahme der
Cauchy--Lipschitz'schen Bedingung. \emph{Monatshefte f\"ur Mathematik und
Physik}, 9, 331--345.

\bibitem[Smeenk and W\"uthrich(2021)]{SmeenkWuthrich}
Smeenk, C., \& W\"uthrich, C. (2021). Determinism and general relativity.
\emph{Philosophy of Science}, 88, 638--664. Preprint arXiv:2009.07555 (2020).
The section reference in the text is to the preprint.

\bibitem[Wallach(1948)]{Wallach}
Wallach, S. (1948). The differential equation $y'=f(y)$. \emph{American Journal
of Mathematics}, 70, 345--350.

\bibitem[Wilson(2009)]{Wilson}
Wilson, M. (2009). Determinism and the mystery of the missing physics.
\emph{British Journal for the Philosophy of Science}, 60, 173--193.

\bibitem[Xia(1992)]{Xia}
Xia, Z. (1992). The existence of noncollision singularities in Newtonian
systems. \emph{Annals of Mathematics}, 135, 411--468.
