...
\smallskip
\emph{The locality claims.} First, (H2) is decided by the restriction of $f$ to
...
Second, (H3) is not decided by the restriction of $f$ to any bounded set. Every
...
lies in some $[-M,M]$, no bounded set decides (H3).
\end{proof}
...
each of the two verdicts is decided: under (H1), the count of forward-maximal
...
\[
  ...
  \tfrac12 G^{2}=-V
  \quad\text{and}\quad
  \tfrac{d}{dx}\big(\tfrac12 G^{2}\big)=GG'=\sqrt x+x^{2}\ \ (x>0).
\]
...
$\ddot x\equiv0=\sqrt0+0^{2}$, so assume otherwise. On the interior of $Z$ we
have $\dot x\equiv0$ and hence $\ddot x\equiv0=\sqrt0+0^{2}$. On
