...
\noindent\textbf{Keywords}\quad Determinism $\cdot$ Indeterminism $\cdot$
Ordinary differential equations $\cdot$ Non-uniqueness $\cdot$ Finite-time
escape
...
``throughout''. On Norton's dome \citep{Norton} the wait-then-depart branches are
defined at every later time, so the two classes coincide and Readings~1 and~2
agree that the state has many futures; in the non-collision singularities of \citet{Xia} the field is locally Lipschitz away from collisions, so finite-time
...
discussed at length by \citet{Malament} and \citet{Fletcher};
see also \citet{BhatBernstein}.
