...
verdicts is decided. \textbf{The example does not by itself establish that
...
each of the two verdicts is decided. The next proposition does no more than
...
\begin{proposition}[Where each verdict is decided]\label{prop:locality}
...
\]
The condition on the left is decided by the restriction of $f$ to any
neighbourhood of the origin. The condition on the right is not decided by the
restriction of $f$ to any bounded set.
\end{proposition}

\begin{proof}
Three cases exhaust the possibilities. If (H2) fails, the departure criterion of
Section~\ref{sec:classical} leaves $x\equiv0$ as the only solution, so
$\Hmax(0)=\Hglob(0)=\{x_{\infty}\}$ and both counts are $1$. If (H2) and (H3)
both hold, Theorem~\ref{thm:counts} gives $|\Hmax(0)|=\cont$ and
$|\Hglob(0)|=1$. If (H2) holds and (H3) fails, then $F$ is a bijection from
...
\{x_{\infty}\}$, of cardinality $\cont$. Reading the two counts off the three
cases gives the two equivalences.

For the locality claims: (H2) is decided by $f$ on $(0,\delta)$ for every
$\delta>0$, because $1/f$ is continuous, hence integrable, on $[\delta,1]$. (H3)
is not decided by $f$ on any bounded set. It is enough to exhibit one pair.
Take $f(x)=|x|^{2/3}+x^{2}$ as in Section~\ref{sec:example} and put
\[
  f_{\flat}(x)=\begin{cases}
    |x|^{2/3}+x^{2}, & |x|\le 1,\\[2pt]
    2|x|, & |x|\ge 1 .
  \end{cases}
\]
Since $f(\pm1)=2$, $f_{\flat}$ is continuous, and it satisfies (H1). It agrees
with $f$ on $[-1,1]$, so it satisfies (H2) for the same reason $f$ does; but
$\int_{1}^{\infty}dx/(2x)=\infty$, so (H3) fails for it. Hence $f$ and
$f_{\flat}$ have the same restriction to $[-1,1]$ while
$|\Hglob(0)|=1$ for $f$ and $|\Hglob(0)|=\cont$ for $f_{\flat}$; the maximal
count is $\cont$ for both.
\end{proof}
...
at all zeroes of $b$''. His case (6.5) corresponds to (H2) and (H3) together,
...
infinity for at least one solution''; his case (6.2), where the integral
...
% TODO: confirm p.~193 and the display number (6.6) against the offprint before
% submission. Checked there by the author; re-check at proof stage.
...
we record it as a constraint and not as a reason to prefer either. One point is common to all three readings: the
...
the point of the example is not a new theorem about differential equations and
not a new criterion of determinism. Two things are offered. The first is the
...
general relativity, $N$-body mechanics or partial differential equations. Where
the two failure modes are usually illustrated separately --- Norton's dome
\cite{Norton}, and the class it specialises \cite{BhatBernstein}, for
non-uniqueness; the non-collision
singularities of Xia \cite{Xia} for finite-time escape --- here they occur in
one equation. The second is Proposition~\ref{prop:locality}, which says where
each of the two verdicts is decided: the maximal-history count is fixed by $f$
near the initial state, the global count is not. The ingredients of that
proposition are the classical dichotomies of
Section~\ref{sec:classical}, and we claim no priority for it.
...
the depth below it, giving $\ddot r=g\,h'(r)$. The example has antecedents in a
class of systems given earlier by Bhat and Bernstein \cite{BhatBernstein}, and
the construction has been discussed at length by Malament \cite{Malament} and
Fletcher \cite{Fletcher}.
% TODO: the relation to Bhat--Bernstein is stated here only as antecedence. The
% dome is often described as a limiting case of their class; that stronger
% statement has not been checked against their article and is not made. The obvious candidate here,
...
extension $\hat G(x)=G(|x|)$ --- an extension of the vector field, not something
derived from a potential on $x<0$, where $V$ is not defined --- is continuous
with $\hat G(0)=0$ and $\hat G>0$ off the origin; $G(x)\asymp x^{3/4}$ as $x\to0^{+}$ and $\asymp x^{3/2}$ as
...
% article before submission. Cited here for the prior class of examples only;
