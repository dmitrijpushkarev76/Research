"""
Symbolic and numerical checks for "One State, Many Maximal Futures".
...
check("sec. 2 sanity check: RK4 from x = 1e-12 to x = 1e7 agrees with the exact "
...
check("sec. 2: that elapsed time falls short of T* by the head and tail the "
...
f_a = u / sp.log(1 / u)
check("ratio counterexample (i): f(u) = u/log(1/u) has f(u)/u -> 0 at 0+ "
...
check("ratio counterexample (i): yet int_0^(1/2) du/f(u) DIVERGES, so (H2) fails -- "
      "the ratio condition does not imply (H2)",
      Ia == sp.oo or Ia.has(sp.oo),
      f"int = {Ia}")

f_b = u * sp.log(u)
check("ratio counterexample (ii): f(u) = u*log(u) has f(u)/u -> +oo at infinity "
...
check("ratio counterexample (ii): yet int_2^inf du/(u log u) DIVERGES, so (H3) fails -- "
...
check("ratio condition, forward direction: if f(u) <= C*u near 0 then int_0 du/f diverges "
...
check("App. A counterexample: for f(x) = |x|^(2/3) on the negative half-line, "
...
check("sec. 2 (iv) near-origin for the witness: int_0^1 3du/(1+u^4) <= 3 < oo, and quadrature "
...
check("sec. 2 (iv) at-infinity for the witness: int_1^inf 3u^-4 du = 1 < oo, and quadrature "
...
check("App. A reflection for the witness: f is even, so int_(-inf)^(-1) dx/f equals the "
...
check("sec. 2: the two pieces sum to T*, which is the arithmetic the closed form "
      "must satisfy",
...
r_dome = (t - T)**4 / 144
check("comparison / Norton dome: r = (t-T)^4/144 solves r'' = +sqrt(r)  [C1 holds]",
      sp.simplify(sp.diff(r_dome, t, 2) - sp.sqrt(sp.Abs(r_dome))) == 0,
      f"r'' = {sp.simplify(sp.diff(r_dome, t, 2))}")
check("comparison / Norton dome: that branch is a POLYNOMIAL, hence defined for all t -- "
...
check("comparison / Earman 3.5: V = -(9/2)x^(4/3) gives force = 6*x^(1/3) on x>0",
      sp.simplify(force - 6 * x**sp.Rational(1, 3)) == 0, f"force = {force}")
check("comparison / Earman 3.5: x = (t-k)^3 satisfies x'' = 6*x^(1/3) on the departing "
...
check("comparison / Earman 3.5: the branch is odd in w, so the mirror branch -(t-k)^3 "
      "solves the same equation with the odd force 6*sgn(x)|x|^(1/3) -- both are global",
      sp.simplify(sp.diff(-x_e, w, 2) - (-6) * x_e**sp.Rational(1, 3)) == 0,
      f"(-x)'' = {sp.diff(-x_e, w, 2)}")

# ============================ 6. Proposition 10: the dome no-go, sec. 11.1
check("App. B.1: arc-length parametrisation rho'^2 + z'^2 = 1 forces |z'| <= 1 "
...
check("App. B.1: the candidate h(r) = (1/g)(2/3 r^(3/2) + 1/3 r^3) does give "
      "r'' = g h'(r) = sqrt(r) + r^2",
...
check("App. B.1: with g = 9.8 the candidate violates |h'| <= 1 beyond r ~ 2.85, "
...
check("App. B.1: |r''| <= g therefore bounds growth quadratically -- integrating "
...
check("App. B.1 caveat: theta is cyclic, so p_theta = m*rho(r)^2*thetadot is conserved",
...
check("App. B.1 caveat: with angular momentum the meridional equation is "
      "r'' = g h'(r) + (p_theta^2/m^2) * rho'(r)/rho(r)^3, NOT r'' = g h'(r)",
      sp.simplify(rddot - predicted) == 0,
      f"r'' = {sp.simplify(rddot)}")

check("App. B.1 caveat: that centrifugal term is unbounded as rho -> 0, so the "
...
check("App. A: (a) does NOT follow from the forward conditions: f(u) = u^(2/3) for u>0 and f(u) = |u| for "
...
check("App. A (a)-counterexample: 1/(v+v^2) has elementary antiderivative "
...
check("App. A (a)-counterexample: f is continuous at 0 from both sides, so (H1) "
...
check("App. B.2: G G' = sqrt(x) + x^2, so solutions of x' = G(x) solve x'' = sqrt(x) + x^2",
      sp.simplify(G * sp.diff(G, x) - (sp.sqrt(x) + x**2)) == 0,
      f"G G' = {sp.simplify(G*sp.diff(G, x))}")
check("App. B.2: G(0) = 0 and G > 0 on (0, oo), so (H1) holds on the half-line",
      sp.limit(G, x, 0, '+') == 0 and G.subs(x, 1) > 0,
      f"G(0+) = {sp.limit(G, x, 0, '+')}, G(1) = {sp.simplify(G.subs(x, 1))}")
check("App. B.2: G(x) ~ x^(3/4) as x -> 0+, exponent 3/4 < 1, so (H2) holds",
...
check("App. B.2: G(x) ~ x^(3/2) as x -> oo, exponent 3/2 > 1, so (H3) holds",
...
check("App. B.2: int_0^inf v^(-5/6)(1+v)^(-1/2) dv = B(1/6, 1/3) EXACTLY "
...
check("App. B.2: the closed form (2/3)sqrt(3/2)2^(-1/3)B(1/6,1/3) agrees with direct "
...
check("App. B.2: the manuscript's decimal 5.4521363617 matches to 10 places",
      abs(sp.N(T_mech_closed, 25) - sp.Rational(54521363617, 10**10)) < sp.Rational(1, 10**10),
      f"T*_mech = {sp.N(T_mech_closed, 14)}")

check("App. B.2: T*_mech differs from T* -- the mechanical lifetime is NOT the "
